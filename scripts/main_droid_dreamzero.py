...
class Args:
    # Hardware parameters
    left_camera_id: str = "31780776"  # e.g., "24259877"
    right_camera_id: str = "36064187"  # e.g., "24514023"
    wrist_camera_id: str = "17967083"  # e.g., "13062452"
    ...
    observation_space: str = "joint_position"
...
def main(args: Args):
    ...
    env = RobotEnv(action_space=args.action_space, gripper_action_space="position")
    ...
    while True:
        ...
        for t_step in bar:
            ...
            try:
                # if t_step == 0:
                #     print("Replay initial position")
                #     initial_action = np.array([ 0.01800841, -0.6926409,  -0.18777452, -2.46676326, -0.01837285,  1.90200365, -0.30914086,  0.        ], dtype=np.float64)
                #     env.update_robot(initial_action, action_space="joint_position", blocking=True)  
                #     time.sleep(1)              
                ...
                if actions_from_chunk_completed == 0 or actions_from_chunk_completed >= args.open_loop_horizon:
                    ...
                    # and improve latency.
                    # tqdm.tqdm.write(f"{curr_obs.keys()}")

                    # assert observation/state.shape == (_, 7)
                    if "joint" in args.observation_space:
                        assert curr_obs[args.observation_space].shape[0] == 7 ,"droid joint state dim is 7, not include gripper"
                    elif "cartesian" in args.observation_space:
                        # print(curr_obs[args.observation_space].shape)
                        assert curr_obs[args.observation_space].shape[0] == 6 ,"droid cartesian state dim is 6, not include gripper"
                    else:
                        # Raise a specific ValueError for unsupported action space
                        raise ValueError(
                            f"Unsupported observation_space space: '{args.observation_space}'. "
                            "Only 'joint' or 'cartesian' observation_space spaces are allowed for droid observation_space."
                        )
                    ...
                    if "joint" in args.action_space:
                        assert pred_action_chunk.shape[1] == 8 ,"droid joint action dim is 8"
                    elif "cartesian" in args.action_space:
                        assert pred_action_chunk.shape[1] == 7 ,"droid cartesian action dim is 7"
                    else:
                        # Raise a specific ValueError for unsupported action space
                        raise ValueError(
                            f"Unsupported action space: '{args.action_space}'. "
                            "Only 'joint' or 'cartesian' action spaces are allowed for droid actions."
                        )
                ...
                if "velocity" in args.action_space:
        ...
        df = pd.concat( [ df, pd.DataFrame( [{ "success": success, "duration": t_step, "video_filename": save_filename, }] ) ], ignore_index=True )
